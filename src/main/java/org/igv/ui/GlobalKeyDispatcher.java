...
public class GlobalKeyDispatcher implements KeyEventDispatcher {
    ...
    private void nextFeature(boolean forward) {
        ...
        Collection<Track> tracks = IGV.getInstance().getSelectedTracks().stream()
                .filter(t -> t instanceof FeatureTrack || t instanceof VariantTrack)
        ...
        if (tracks.size() == 1) {
            ...
            MessageUtils.showMessage("<html>To jump between features your must first select a feature or variant track<br/>" +
                    "by clicking on the track name or using the selection checkboxes.");
        }
