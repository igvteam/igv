...
public class SyntenyUtils
{
    ...
    public static void main(String[] args) throws IOException
    {
        ...
        mapSegments(is, inputFile, outputFile);
        is.close();
    }
    
    public static void mapCNFile(InputStream mappingStream, String cnFile, String oFile)
    {
        Map mappings = loadMappings(mappingStream, true);
    ...
    private static void mapSegments(InputStream mappingFile, String iFile, String oFile)
    {
        ...
        try {
            ...
            while (((nextLine = reader.readLine()) != null) && (nextLine.trim().length() > 0))
            {
                ...
                if (nTokens == 1) {
                    ...
                    value = Float.parseFloat(tokens[valueColumn]);
                    
                    List tmp = (List)mappings.get(chr);
                    if (tmp == null) {
                    ...
                    if (overlappingMappings == null) {
                        ...
                        for (Mapping mapping : overlappingMappings)
                        {
                            ...
                            pw.println(sample + "\t" + mapping.getToChr() + "\t" + toStart + "\t" + toEnd + "\t" + snpCount + "\t" + value);
                        }
                }
            }
        }
    ...
    public static void toBed(InputStream mappingFile, String ofile)
    {
        PrintWriter pw = null;
        try
        {
    ...
    public static Map<String, List<Mapping>> loadMappings(String path, boolean reverse) throws IOException {

        InputStream is = null;
        try {
            is = ParsingUtils.openInputStream(path);
            return loadMappings(is, reverse);
        }
        finally {
            if(is != null) {
                is.close();
    ...
    public static Map<String, List<Mapping>> loadMappings(InputStream stream, boolean reverse)
    {
        BufferedReader reader = null;
        ...
        try
        {
            ...
            while ((nextLine = reader.readLine()) != null) {
                ...
                Mapping syntenyMapping = (Mapping)c.newInstance();
                if (reverse) {
    ...
    private static int getIndexBefore(List<Mapping> values, int x, int leftBound, int rightBound)
    {
        ...
        if (((Mapping)values.get(idx)).getFromStart() == x) {
            return idx;
        }
        
        if (((Mapping)values.get(idx)).getFromStart() < x) {
            if (((Mapping)values.get(idx + 1)).getFromStart() >= x) {
                return idx;
            }
            leftBound = idx;
