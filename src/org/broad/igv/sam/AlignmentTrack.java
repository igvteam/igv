 ...
 * @author jrobinso
 */
public class AlignmentTrack extends AbstractTrack implements
		AlignmentTrackEventListener {
	...
	static {
		bisulfiteContextToContextString.put(BisulfiteContext.CG,
				new Pair<byte[], byte[]>(new byte[] {}, new byte[] { 'G' }));
		bisulfiteContextToContextString
				.put(BisulfiteContext.CHH, new Pair<byte[], byte[]>(
						new byte[] {}, new byte[] { 'H', 'H' }));
		bisulfiteContextToContextString
				.put(BisulfiteContext.CHG, new Pair<byte[], byte[]>(
						new byte[] {}, new byte[] { 'H', 'G' }));
		bisulfiteContextToContextString
				.put(BisulfiteContext.HCG, new Pair<byte[], byte[]>(
						new byte[] { 'H' }, new byte[] { 'G' }));
		bisulfiteContextToContextString
				.put(BisulfiteContext.GCH, new Pair<byte[], byte[]>(
						new byte[] { 'G' }, new byte[] { 'H' }));
		bisulfiteContextToContextString
				.put(BisulfiteContext.WCG, new Pair<byte[], byte[]>(
						new byte[] { 'W' }, new byte[] { 'G' }));
	}
	...
	// The "parent" of the track (a DataPanel). This field might be null at any
	// given time. It is updated each repaint.
	JComponent parent;
	private Rectangle alignmentsRect;
	private Rectangle downsampleRect;
	final static String m_filterAlignment = "Filter Alignments";
	 ...
	 * @param genome
	 */
	public AlignmentTrack(ResourceLocator locator,
			AlignmentDataManager dataManager, Genome genome) {
		super(locator);
		...
		dataManager.setShowSpliceJunctions(prefs
				.getAsBoolean(PreferenceManager.SAM_SHOW_JUNCTION_TRACK));

		float maxRange = prefs
				.getAsFloat(PreferenceManager.SAM_MAX_VISIBLE_RANGE);
	...
	public void setSpliceJunctionTrack(
	...
	public int getHeight() {
		...
		if (parent != null
				&& (parent instanceof DataPanel && ((DataPanel) parent)
						.getFrame().getScale() > minVisibleScale)) {
			return minimumHeight;
		}

		int nGroups = dataManager.getMaxGroupCount();

		int h = Math.max(minHeight, getNLevels() * getRowHeight() + nGroups
				* GROUP_MARGIN + TOP_MARGIN + DS_MARGIN_0
				+ DOWNAMPLED_ROW_HEIGHT + DS_MARGIN_2);
	...
	private int getRowHeight() {
		return getDisplayMode() == DisplayMode.EXPANDED ? expandedHeight
				: squishedHeight;
	}
	...
	public void render(RenderContext context, Rectangle rect) {
		...
		if (context.getScale() > minVisibleScale) {
			...
			GraphicUtils.drawCenteredText("Zoom in to see alignments.",
					visibleRect, g);
			return;
		}
	...
	private void renderDownsampledIntervals(RenderContext context,
			Rectangle downsampleRect) {

		// Might be offscreen
		if (!context.getVisibleRect().intersects(downsampleRect))
			return;

		final AlignmentInterval loadedInterval = dataManager
				.getLoadedInterval(context.getReferenceFrame());
		if (loadedInterval == null)
			return;

		Graphics2D g = context.getGraphic2DForColor(Color.black);
		List<CachingQueryReader.DownsampledInterval> intervals = loadedInterval
				.getDownsampledIntervals();
		for (CachingQueryReader.DownsampledInterval interval : intervals) {
			...
			// Color color = (gray <= 0 ? Color.black :
			// ColorUtilities.getGrayscaleColor(gray));
	...
	private void renderAlignments(RenderContext context, Rectangle inputRect) {
		try {
			...
			Map<String, List<AlignmentInterval.Row>> groups = dataManager
					.getGroups(context, renderOptions,
			...
			// Loop through groups
			Graphics2D groupBorderGraphics = context
					.getGraphic2DForColor(AlignmentRenderer.GROUP_DIVIDER_COLOR);
			int nGroups = groups.size();
			int groupNumber = 0;
			for (Map.Entry<String, List<AlignmentInterval.Row>> entry : groups
					.entrySet()) {
				...
				for (AlignmentInterval.Row row : rows) {
					...
					if (y + h > visibleRect.getY()) {
						Rectangle rowRectangle = new Rectangle(inputRect.x,
								(int) y, inputRect.width, (int) h);
						renderer.renderAlignments(row.alignments, context,
								rowRectangle, inputRect, renderOptions,
								leaveMargin, selectedReadNames);
					}
					y += h;
				}
				...
				if (groupNumber < nGroups) {
					...
					groupBorderGraphics.drawLine(inputRect.x, borderY,
							inputRect.width, borderY);
				}
				y += GROUP_MARGIN;
			}

			final int bottom = inputRect.y + inputRect.height;
			groupBorderGraphics.drawLine(inputRect.x, bottom, inputRect.width,
					bottom);
	 ...
	 */
	public void groupAlignments(GroupOption option,
			ReferenceFrame referenceFrame) {
		if (renderOptions.groupByOption != option) {
			renderOptions.groupByOption = (option == GroupOption.NONE ? null
					: option);
	 ...
	 */
	public void copyToClipboard(final TrackClickEvent e, Alignment alignment,
			double location) {
		...
		if (alignment != null) {
			...
			buf.append(alignment.getValueString(location, null).replace("<br>",
					"\n"));
			buf.append("\n");
			buf.append("Alignment start position = " + alignment.getChr() + ":"
					+ (alignment.getAlignmentStart() + 1));
			buf.append("\n");
	...
	public void gotoMate(final TrackClickEvent te, Alignment alignment) {
		...
		if (alignment != null) {
			...
			if (mate != null && mate.isMapped()) {
				...
				MessageUtils
						.showMessage("Alignment does not have mate, or it is not mapped.");
			}
		}
	}
	...
	public void splitScreenMate(final TrackClickEvent te, Alignment alignment) {
		...
		if (alignment != null) {
			...
			if (mate != null && mate.isMapped()) {
				...
				// Generate a locus string for the read mate. Keep the window
				// width (in base pairs) == to the current range
				...
				// If we are already in gene list mode add the mate as another
				// panel, otherwise switch to gl mode
				...
				GeneList geneList = new GeneList(listName.toString(), loci,
						false);
	...
	public String getValueStringAt(String chr, double position, int y,
		...
		if (downsampleRect != null && y > downsampleRect.y
				&& y <= downsampleRect.y + downsampleRect.height) {
			AlignmentInterval iv = dataManager.getLoadedInterval(frame);
			if (iv == null) {
				return null;
			} else {
				List<CachingQueryReader.DownsampledInterval> intervals = iv
						.getDownsampledIntervals();
				CachingQueryReader.DownsampledInterval interval = (CachingQueryReader.DownsampledInterval) FeatureUtils
						.getFeatureAt(position, 0, intervals);
			...
			SortedSet<Shape> arcs = AlignmentRenderer.getInstance()
					.curveOverlap(xloc);
			int halfLength = 2;
			int sideLength = 2 * halfLength;
			for (Shape curve : arcs) {
				if (curve.intersects(xloc - halfLength, y - halfLength,
						sideLength, sideLength)) {
					feature = AlignmentRenderer.getInstance()
							.getAlignmentForCurve(curve);
					break;
				}
			}
			return feature == null ? null : feature.getValueString(position,
					getWindowFunction());
		} else {
	...
	private Alignment getAlignmentAt(double position, int y,
		...
		Map<String, List<AlignmentInterval.Row>> groups = dataManager
				.getGroupedAlignments(frame);

		if (groups == null || groups.isEmpty()) {
			return null;
		}

		int h = getDisplayMode() == DisplayMode.EXPANDED ? expandedHeight
				: squishedHeight;
		...
		for (List<AlignmentInterval.Row> rows : groups.values()) {
			...
			if (y >= startY && y < endY) {
				...
				// No buffer for alignments, you must zoom in far enough for
				// them to be visible
				int buffer = 0;
				return (Alignment) FeatureUtils.getFeatureAt(position, buffer,
						features);
			}
	...
	public void onAlignmentTrackEvent(AlignmentTrackEvent e) {
		...
		case SPLICE_JUNCTION:
			final boolean showJunctions = PreferenceManager.getInstance()
					.getAsBoolean(PreferenceManager.SAM_SHOW_JUNCTION_TRACK);
	...
	public boolean handleDataClick(TrackClickEvent te) {
		...
		if (Globals.IS_MAC && e.isMetaDown()
				|| (!Globals.IS_MAC && e.isControlDown())) {
			// Selection
	...
	private void setSelected(Alignment alignment) {
		Color c = alignment.isPaired() && alignment.getMate() != null
				&& alignment.getMate().isMapped() ? ColorUtilities
				.randomColor(selectionColorIndex++) : Color.black;
	...
	public static boolean isBisulfiteColorType(ColorOption o) {
		return (o.equals(ColorOption.BISULFITE) || o
				.equals(ColorOption.NOMESEQ));
	}
	...
	public static byte[] getBisulfiteContextPreContext(BisulfiteContext item) {
		Pair<byte[], byte[]> pair = AlignmentTrack.bisulfiteContextToContextString
				.get(item);
	...
	public Map<String, String> getPersistentState() {
		...
		if (dataManager.isShowSpliceJunctions()) {
			attrs.put("showSpliceJunctions",
					String.valueOf(dataManager.isShowSpliceJunctions()));
		}
	...
	public void restorePersistentState(Map<String, String> attributes) {
		...
		if (spliceJunctionOption != null) {
			try {
				dataManager.setShowSpliceJunctions(Boolean
						.parseBoolean(spliceJunctionOption));
			} catch (Exception e) {
				log.error("Error restoring splice junction option: "
						+ spliceJunctionOption);
			}
		}
	...
	public void setViewAsPairs(boolean vAP) {
		...
		if (vAP && renderOptions.groupByOption == GroupOption.STRAND) {
			boolean ungroup = MessageUtils
					.confirm("\"View as pairs\" is incompatible with \"Group by strand\". Ungroup?");
			if (ungroup) {
	...
	public static class RenderOptions {
		...
		List<AlignmentFilter> alnFilter;

		public List<AlignmentFilter> getAlnFilter() {
			return alnFilter;
		}

		public void setAlnFilter(List<AlignmentFilter> alnFilter) {
			this.alnFilter = alnFilter;
		}

		RenderOptions() {
			PreferenceManager prefs = PreferenceManager.getInstance();
			shadeBases = prefs
					.getAsBoolean(PreferenceManager.SAM_SHADE_BASE_QUALITY);
			shadeCenters = prefs
					.getAsBoolean(PreferenceManager.SAM_SHADE_CENTER);
			flagUnmappedPairs = prefs
					.getAsBoolean(PreferenceManager.SAM_FLAG_UNMAPPED_PAIR);
			computeIsizes = prefs
					.getAsBoolean(PreferenceManager.SAM_COMPUTE_ISIZES);
			minInsertSize = prefs
					.getAsInt(PreferenceManager.SAM_MIN_INSERT_SIZE_THRESHOLD);
			maxInsertSize = prefs
					.getAsInt(PreferenceManager.SAM_MAX_INSERT_SIZE_THRESHOLD);
			minInsertSizePercentile = prefs
					.getAsFloat(PreferenceManager.SAM_MIN_INSERT_SIZE_PERCENTILE);
			maxInsertSizePercentile = prefs
					.getAsFloat(PreferenceManager.SAM_MAX_INSERT_SIZE_PERCENTILE);
			showAllBases = DEFAULT_SHOWALLBASES;
			colorOption = ColorOption.strToValue(prefs
					.get(PreferenceManager.SAM_COLOR_BY));
			groupByOption = null;
			flagZeroQualityAlignments = prefs
					.getAsBoolean(PreferenceManager.SAM_FLAG_ZERO_QUALITY);
			...
			// A class that can handle filters for filtering out alignments
			alnFilter = null;
		}
		...
		public Map<String, String> getPersistentState() {
			...
			if (shadeCenters != prefs
					.getAsBoolean(PreferenceManager.SAM_SHADE_CENTER)) {
				attributes.put("shadeCenters", String.valueOf(shadeBases));
			}
			if (flagUnmappedPairs != prefs
					.getAsBoolean(PreferenceManager.SAM_FLAG_UNMAPPED_PAIR)) {
				attributes.put("flagUnmappedPairs",
						String.valueOf(flagUnmappedPairs));
			}
			if (maxInsertSize != prefs
					.getAsInt(PreferenceManager.SAM_MAX_INSERT_SIZE_THRESHOLD)) {
				attributes.put("insertSizeThreshold",
						String.valueOf(maxInsertSize));
			}
			if (getMinInsertSize() != prefs
					.getAsInt(PreferenceManager.SAM_MIN_INSERT_SIZE_THRESHOLD)) {
				attributes.put("minInsertSizeThreshold",
						String.valueOf(maxInsertSize));
			}
		...
		public void setColorByTag(String colorByTag) {
			...
			PreferenceManager.getInstance().put(
					PreferenceManager.SAM_COLOR_BY_TAG, colorByTag);
		}
	...
	class FilterOptions implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getActionCommand().startsWith("remove")) {
				...
				clonedFilter.remove(idx.intValue());
			...
			if (e.getActionCommand().startsWith("up")) {
				...
				String id = jrb.getName();
				Integer idx = getIdxFromId(id);
				AlignmentFilter tempFilter = clonedFilter.get(idx);
				clonedFilter.remove(idx.intValue());
				if (idx < 1)
					idx = 1;
				clonedFilter.add(idx - 1, tempFilter);
				dialogBounds = dialog.getBounds();
				dialogLocation = dialog.getLocation();
				dialog.dispose();
				selectFilters(selectedTracks);
			}else
			if (e.getActionCommand().startsWith("down")) {
				JButton jrb = (JButton) e.getSource();
				String id = jrb.getName();
				Integer idx = getIdxFromId(id);
				AlignmentFilter tempFilter = clonedFilter.get(idx);
				clonedFilter.remove(idx.intValue());
				if (idx+1 >= clonedFilter.size())
					idx = clonedFilter.size()-1;
				clonedFilter.add(idx+1, tempFilter);
				dialogBounds = dialog.getBounds();
				dialogLocation = dialog.getLocation();
				dialog.dispose();
			...
			if (e.getActionCommand().startsWith("apply")) {
				...
				String id = jrb.getName();
				Integer idx = getIdxFromId(id);
				AlignmentFilterPanel thisFilter = (AlignmentFilterPanel) allFilterPanel
						.getComponent(idx);

				clonedFilter.get(idx).setExclude(thisFilter.getExcludeButton());
				clonedFilter.get(idx).setTag(thisFilter.getTagValue());
				clonedFilter.get(idx).setFilterType(thisFilter.getFilterType());
				clonedFilter.get(idx).setExpression(
						thisFilter.getExpressionString());
				clonedFilter.get(idx).setHasWildCards(
						thisFilter.getHasWildCardsButton());
				clonedFilter.get(idx).setRegExpr(
						thisFilter.getIsRegExprButton());
				clonedFilter.get(idx).setCaseSensitive(
						thisFilter.getCaseSensitiveBox());
				clonedFilter.get(idx).setMin(thisFilter.getMinField());
				clonedFilter.get(idx).setMax(thisFilter.getMaxField());
		...
		private Integer getIdxFromId(String id) {
			...
			out = Integer.parseInt(idParts[idParts.length - 1]);

			return out;
		}

		protected void selectFilters(Collection<Track> selectedTracks) {
			...
			if (firstTime) {
				...
				if (clonedFilter == null) {
					clonedFilter = new ArrayList<AlignmentFilter>();
				}
				if (renderOptions.alnFilter != null) {
					for (Integer i = 0; i < renderOptions.alnFilter.size(); i++) {
						allFilterPanel.add(paintAlignmentFilterDialog(
								renderOptions.alnFilter.get(i), i.toString()));
					}
				}
				firstTime = false;
			} else {
				for (Integer i = 0; i < clonedFilter.size(); i++) {
					allFilterPanel.add(paintAlignmentFilterDialog(
							clonedFilter.get(i), i.toString()));
				}
			}
			// clonedFilter.add(new AlignmentFilter());

			newAlnFilter = new AlignmentFilter();
			JPanel newP = paintAlignmentFilterDialog(newAlnFilter, "new");
			allFilterPanel.add(newP);
			...
			Object[] options = { "save filters", "Cancel" };
			JOptionPane pane = new JOptionPane(scrollPane,
					JOptionPane.PLAIN_MESSAGE, JOptionPane.YES_OPTION, null,
					options, options[0]);

			dialog = pane.createDialog(null, "Filter settings");
		...
		private AlignmentFilterPanel paintAlignmentFilterDialog(
				AlignmentFilter alf, String index) {
			AlignmentFilterPanel newP = new AlignmentFilterPanel(alf, index,
					this);
	...
	class PopupMenu extends IGVPopupMenu {
		...
		PopupMenu(final TrackClickEvent e) {
			...
			JLabel popupTitle = new JLabel(
					"  " + AlignmentTrack.this.getName(), JLabel.CENTER);
			...
			boolean viewPairArcsPresent = Boolean.parseBoolean(System
					.getProperty("pairedArcViewPresent", "false"));
		...
		private JMenu getBisulfiteContextMenuItem(ButtonGroup group) {
			...
			boolean showNomeESeq = PreferenceManager.getInstance()
					.getAsBoolean(PreferenceManager.SAM_NOMESEQ_ENABLED);
			if (showNomeESeq) {
				nomeESeqOption = new JRadioButtonMenuItem(
						"NOMe-seq bisulfite mode");
				nomeESeqOption
						.setSelected(renderOptions.colorOption == ColorOption.NOMESEQ);
		...
		public void addSelecteByNameItem() {
			...
			item.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent aEvt) {
					String val = MessageUtils
							.showInputDialog("Enter read name: ");
					if (val != null && val.trim().length() > 0) {
						selectedReadNames.put(val, ColorUtilities
								.randomColor(selectedReadNames.size() + 1));
						refresh();
					}
				}
			});
		...
		private JCheckBoxMenuItem getGroupMenuItem(String label,
				final GroupOption option) {
		...
		public void addGroupMenuItem() {// ReferenceFrame frame) {
			...
			mappings.put("first-in-pair strand",
					GroupOption.FIRST_OF_PAIR_STRAND);
			...
			for (Map.Entry<String, GroupOption> el : mappings.entrySet()) {
				JCheckBoxMenuItem mi = getGroupMenuItem(el.getKey(),
						el.getValue());
				groupMenu.add(mi);
				group.add(mi);
			}
			...
			tagOption.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent aEvt) {
					String tag = MessageUtils.showInputDialog("Enter tag",
							renderOptions.getGroupByTag());
			...
			tagOption
					.setSelected(renderOptions.groupByOption == GroupOption.TAG);
		...
		public void addSortMenuItem() {
			...
			// LinkedHashMap is supposed to preserve order of insertion for
			// iteration
			...
			mappings.put("first-of-pair strand",
					SortOption.FIRST_OF_PAIR_STRAND);
		...
		private void setBisulfiteContext(BisulfiteContext option) {
			...
			PreferenceManager.getInstance().put(
					PreferenceManager.SAM_BISULFITE_CONTEXT, option.toString());
		}

		private void setColorOption(ColorOption option) {
			renderOptions.colorOption = option;
			PreferenceManager.getInstance().put(PreferenceManager.SAM_COLOR_BY,
					option.toString());

			// TODO Setting "color-by bisulfite" also controls the experiment
			// type. This is temporary, until we
			// expose experimentType directory.
			ExperimentType t = (option == ColorOption.BISULFITE ? ExperimentType.BISULFITE
					: ExperimentType.OTHER);
			setExperimentType(t);

		}

		private JRadioButtonMenuItem getColorMenuItem(String label,
		...
		public void addColorByMenuItem() {
			...
			if (dataManager.isPairedEnd()) {
				mappings.put("first-of-pair strand",
						ColorOption.FIRST_OF_PAIR_STRAND);
			}
			...
			for (Map.Entry<String, ColorOption> el : mappings.entrySet()) {
				JRadioButtonMenuItem mi = getColorMenuItem(el.getKey(),
						el.getValue());
				colorMenu.add(mi);
				group.add(mi);
			}
		...
		public void addCopyToClipboardItem(final TrackClickEvent te) {
			...
			if (frame == null) {
				...
				final Alignment alignment = getAlignmentAt(location, me.getY(),
						frame);
		...
		public void addGoToMate(final TrackClickEvent te) {
			...
			if (frame == null) {
				...
				double location = frame.getChromosomePosition(e.getX());
				final Alignment alignment = getAlignmentAt(location, e.getY(),
						frame);
				item.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent aEvt) {
				...
				if (alignment == null || !alignment.isPaired()
						|| !alignment.getMate().isMapped()) {
					item.setEnabled(false);
				}
			}
			add(item);
		...
		public void addShowAllBasesMenuItem() {
			...
			if (renderOptions.colorOption == ColorOption.BISULFITE
					|| renderOptions.colorOption == ColorOption.NOMESEQ) {
				// item.setEnabled(false);
			} else {
		...
		// JCheckBoxMenuItem("Set maximum coverage depth ...");
		// item.addActionListener(new ActionListener() {
		//
		// public void actionPerformed(ActionEvent aEvt) {
		// int maxLevels = dataManager.getDownsampleOptions().getMaxReadCount();
		// String val = MessageUtils.showInputDialog("Maximum coverage depth",
		// String.valueOf(maxLevels));
		...
		// MessageUtils.showMessage("Insert size must be an integer value: " +
		// val);
		// }
		//
		// }
		...
		public void addInsertSizeMenuItem() {
			...
			item.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent aEvt) {
					...
					InsertSizeSettingsDialog dlg = new InsertSizeSettingsDialog(
							IGV.getMainFrame(), renderOptions);
					dlg.setModal(true);
					dlg.setVisible(true);
					if (!dlg.isCanceled()) {
						renderOptions.setComputeIsizes(dlg.isComputeIsize());
						renderOptions.setMinInsertSizePercentile(dlg
								.getMinPercentile());
						renderOptions.setMaxInsertSizePercentile(dlg
								.getMaxPercentile());
		...
		public void addShowCoverageItem() {
			...
			item.setSelected(getCoverageTrack() != null
					&& getCoverageTrack().isVisible());
			item.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent aEvt) {
					UIUtilities.invokeOnEventThread(new Runnable() {
						...
						public void run() {
							...
							File file = FileDialogUtils.chooseFile(
									"Select coverage file", initDirectory,
									FileDialog.LOAD);
							if (file != null) {
								prefs.setLastTrackDirectory(file
										.getParentFile());
								String path = file.getAbsolutePath();
								if (path.endsWith(".tdf")
										|| path.endsWith(".tdf")) {
									TDFReader reader = TDFReader.getReader(file
											.getAbsolutePath());
									TDFDataSource ds = new TDFDataSource(
											reader, 0, getName() + " coverage",
											genome);
									getCoverageTrack().setDataSource(ds);
									refresh();
								} else if (path.endsWith(".counts")) {
									DataSource ds = new GobyCountArchiveDataSource(
											file);
									getCoverageTrack().setDataSource(ds);
									refresh();
								} else {
									MessageUtils
											.showMessage("Coverage data must be in .tdf format");
								}
							}
						}
					});
