...
public class SamAlignment extends AbstractAlignment implements Alignment {
	...
	private int start; // <= Might differ from alignment start if soft clipping
						// is considered
	 ...
	 */
	protected static final char NT2COMP[] = { 'N', 'N', 'N', 'N', 'N', 'N',
			'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N',
			'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N',
			'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N',
			'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N',
			'N', 'N', 'N', 'N', 'N', 'N', 'N', 'T', 'N', 'G', 'N', 'N', 'N',
			'C', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N',
			'A', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N',
			'T', 'N', 'G', 'N', 'N', 'N', 'C', 'N', 'N', 'N', 'N', 'N', 'N',
	...
	public SamAlignment(SAMRecord record) {
		...
		this.chr = genome == null ? refName : genome
				.getChromosomeAlias(refName);

		// SAMRecord is 1 based inclusive. IGV is 0 based exclusive.
		this.alignmentStart = record.getAlignmentStart() - 1;
		this.start = this.alignmentStart; // might be modified later for soft
											// clipping
		...
		this.firstInPair = record.getReadPairedFlag() ? record
				.getFirstOfPairFlag() : true;
		...
		createAlignmentBlocks(record.getCigarString(), record.getReadBases(),
				...
				getFlowSignals(flowOrder, keySequence), flowOrder,
				this.getFlowSignalsStart());

		Object colorTag = record.getAttribute("YC");
		if (colorTag != null) {
			try {
				defaultColor = ColorUtilities
						.stringToColor(colorTag.toString());
			} catch (Exception e) {
	...
	private void setMatePair(Genome genome) {
		if (record.getReadPairedFlag()) {
			...
			String mateChr = genome == null ? mateReferenceName : genome
					.getChromosomeAlias(mateReferenceName);
			this.properPairFlag = record.getProperPairFlag();
			this.setMate(new ReadMate(mateChr, record.getMateAlignmentStart(),
					record.getMateNegativeStrandFlag(), record
							.getMateUnmappedFlag()));
	...
	private void setPairStrands() {
		...
		if (isPaired()) {
			...
			if (isSecondOfPair()) {
				secondOfPairStrand = isNegativeStrand() ? Strand.NEGATIVE
						: Strand.POSITIVE;
			} else {
				ReadMate mate = getMate();
				if (mate.isMapped() && isProperPair()) {
					secondOfPairStrand = mate.isNegativeStrand() ? Strand.NEGATIVE
							: Strand.POSITIVE;
				} else {
					// No Mate, or mate is not mapped, FOP strand is not defined
			...
			// This alignment is not paired -- by definition "firstOfPair" is
			// this alignment
			firstOfPairStrand = getReadStrand();
			secondOfPairStrand = Strand.NONE;
		}
	...
	private class FlowOrderAndSignalsBlockHelper {
		...
		private boolean readNegativeStrandFlag;
		private boolean[] incorporations = null; // required for the reverse
													// strand

		public FlowOrderAndSignalsBlockHelper(short[] flowSignals,
				String flowOrder, int flowOrderStart, byte[] readBases,
			...
			this.flowSignalsIndex = 0; // NB: the key sequence/barcode sequence
										// should have been remove for the
										// signals already
		...
		public short[][][] createBlockFlowSignals(byte[] readBases,
			...
			i = fromIdx;
			idx = 0;
			while (0 <= this.flowSignalsIndex
					&& this.flowSignalsIndex < this.flowSignals.length
					&& i < fromIdx + nBases) {
				...
				if (j < readBases.length) {
					if (this.readNegativeStrandFlag) {
						...
						// NB: loop condition is not symmetric to the forward,
						// as we must respect the directionality of sequencing.
						// For example, if our flow order is TACAG, and our read
						// bases are TAG, then the flow signal vector is
						// approximately 100,100,0,0,100. Since we move in the
						// reverse direction with respect to the flow signal
						// vector we must pre-compute where the flows
						// incorporations are expected to occur, instead of just
						// looking
						// for the next flow that matches our next read base (we
						// would place the A incorporation flow in the fourth
						// flow,
						// which is wrong).
						while (!this.incorporations[this.flowSignalsIndex]
								|| this.flowOrder.charAt(this.flowOrderIndex) != NT2COMP[readBases[j]]) { // NB:
																											// malicious
																											// input
																											// can
																											// cause
						...
						nextFlowSignalsStart = this.flowSignalsIndex + 1;
						while (this.flowOrder.charAt(this.flowOrderIndex) != readBases[j]) { // NB:
																								// malicious
																								// input
																								// can
																								// cause
				...
				if (0 <= this.prevFlowSignalsStart
						&& this.prevFlowSignalsStart <= this.prevFlowSignalsEnd
						&& this.prevFlowSignalsEnd < this.flowSignals.length) {
					blockFlowSignals[idx][0] = new short[this.prevFlowSignalsEnd
							- this.prevFlowSignalsStart + 1];
					if (this.readNegativeStrandFlag) {
						for (j = this.prevFlowSignalsEnd; this.prevFlowSignalsStart <= j; j--) {
							blockFlowSignals[idx][0][this.prevFlowSignalsEnd
									- j] = this.flowSignals[j];
						}
					} else {
						for (j = this.prevFlowSignalsStart; j <= this.prevFlowSignalsEnd; j++) {
							blockFlowSignals[idx][0][j
									- this.prevFlowSignalsStart] = this.flowSignals[j];
						}
					}
				} else {
				...
				// next context
				if (0 <= nextFlowSignalsStart
						&& nextFlowSignalsStart <= nextFlowSignalsEnd
						&& nextFlowSignalsEnd < this.flowSignals.length) {
					blockFlowSignals[idx][2] = new short[nextFlowSignalsEnd
							- nextFlowSignalsStart + 1];
					if (this.readNegativeStrandFlag) {
	 ...
	 */
	private void createAlignmentBlocks(String cigarString, byte[] readBases,
			byte[] readBaseQualities) {
		createAlignmentBlocks(cigarString, readBases, readBaseQualities, null,
				null, -1);
	}
	 ...
	 * @param readBaseQualities
	 * @param flowSignals
	 *            from the FZ tag, null if not present
	 * @param flowOrder
	 *            from the RG.FO header tag, null if not present
	 * @param flowOrderStart
	 */
	private void createAlignmentBlocks(String cigarString, byte[] readBases,
			byte[] readBaseQualities, short[] flowSignals, String flowOrder,
			int flowOrderStart) {

		boolean showSoftClipped = PreferenceManager.getInstance().getAsBoolean(
		...
		if (cigarString.equals("*")) {
			...
			alignmentBlocks[0] = new AlignmentBlock(getStart(), readBases,
					readBaseQualities, this);
			return;
		}
		...
		for (int i = 0; i < cigarString.length(); i++) {
			...
			if (Character.isDigit(next)) {
				...
				if (operatorIsMatch(showSoftClipped, op)) {
					...
					nInsertions++;
					nGaps++; // "virtual" gap, account for artificial block
								// split @ insertion
				}
		...
		if (null != flowSignals) {
			if (0 < readBases.length) {
				fBlockHelper = new FlowOrderAndSignalsBlockHelper(flowSignals,
						flowOrder, flowOrderStart, readBases,
						this.readNegativeStrandFlag);
			}
		}
		prevOp = 0;
		for (CigarOperator op : operators) {
			try {
				...
				if (operatorIsMatch(showSoftClipped, op.operator)) {
					...
					// TODO -- represent missing sequence ("*") explicitly for
					// efficiency.
					if (readBases == null || readBases.length == 0) {
						...
						System.arraycopy(readBases, fromIdx, blockBases, 0,
								op.nBases);
					}

					nBasesAvailable = readBaseQualities.length - fromIdx;
					if (readBaseQualities == null
							|| readBaseQualities.length == 0
							|| nBasesAvailable < op.nBases) {
						Arrays.fill(blockQualities, (byte) 126);
					} else {
						System.arraycopy(readBaseQualities, fromIdx,
								blockQualities, 0, op.nBases);
					}

					if (null != fBlockHelper) {
						blockFlowSignals = fBlockHelper.createBlockFlowSignals(
								readBases, fromIdx, op.nBases);
						block = AlignmentBlock.getInstance(blockStart,
								blockBases, blockQualities, blockFlowSignals,
								this);
					} else {
						block = AlignmentBlock.getInstance(blockStart,
								blockBases, blockQualities, this);
					}
					...
					// This gap is between blocks split by insertion. It is a
					// zero
	...
	private boolean operatorIsMatch(boolean showSoftClipped, char operator) {
		return operator == MATCH || operator == PERFECT_MATCH
				|| operator == MISMATCH
	...
	String getValueStringImpl(double position, boolean truncate) {
		...
		StringBuffer buf = new StringBuffer(
				super.getValueString(position, null));
					...
					for (String token : tokens) {
						if (token.length() > maxLength) {
							...
							while (remainder.length() > maxLength) {
								...
								int idx = spaceIndex > 30 ? spaceIndex
										: maxLength;
								final String substring = remainder.substring(0,
										idx);
	...
	/**
	 * @return start index in the flow signal as specified by the ZF tag, or -1
	 *         if not present or non-numeric
	 */
	public int getFlowSignalsStart() {
		Object attribute = record.getAttribute(FLOW_SIGNAL_TAG); // NB: from a
																	// TMAP
																	// optional
																	// tag
		int toRet = -1;
	...
	/**
	 * @param flowOrder
	 *            the flow order corresponding to this read
	 * @param keySequence
	 *            sequence the key sequence corresponding to this read
	 * @return the flow signals in 100x format (SFF), only if they exist (FZ
	 *         tag), if the key sequence and flow order are found in the read
	 *         group header tag (RG.KS and RG.FO). Note: the array proceeds in
	...
	public short[] getFlowSignals(String flowOrder, String keySequence) {
		...
		// get the # of bases that the first base in the read overlaps with the
		// last base(s) in the key
		if (this.readNegativeStrandFlag) {
			firstBase = (char) NT2COMP[record.getReadBases()[record
					.getReadLength() - 1]];
		} else {
			firstBase = (char) record.getReadBases()[0];
		}
		keySignalOverlap = 0;
		for (i = keySequence.length() - 1; 0 <= i
				&& keySequence.charAt(i) == firstBase; i--) {
