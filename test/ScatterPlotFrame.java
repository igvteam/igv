...
public class ScatterPlotFrame extends JFrame {
    ...
    * */
    ...
    private void updateModel() {
        ...
        if (isSymbolType(selectedCategory)) {

        }

        String[] seriesNames = null;
        if (selectedCategory == null || isDataCategory(selectedCategory)) {
        ...
        for (String series : seriesNames) {
            XYSeries xySeries = createSeries(series, xAxisName, yAxisName, sampleNames, xValues, yValues,
                    mutationCount, methylation, expression, copyNumber);
    ...
    private XYSeries createSeries(String seriesName, String xAxisName, String yAxisName, String[] sampleNames, double[]
            xValues, double[] yValues, int[] mutationCount, double[] methylation, double[] expression, double[] copyNumber) {
        ...
        for (int idx = 0; idx < xValues.length; ++idx) {
            ...
            XYDataPoint dataPoint = new XYDataPoint(xValues[idx], yValues[idx], tooltip.toString());
            dataPoint.setMutationCount(mutationCount[idx]);
            if (copyNumber != null && copyNumber.length > 0) {
                dataPoint.setCopyNumber(copyNumber[idx]);
            }
            if (expression != null && expression.length > 0) {
                dataPoint.setExpression(expression[idx]);
            }
            if (methylation != null && methylation.length > 0) {
                dataPoint.setMethylation(methylation[idx]);
            }
            xySeries.add(dataPoint);
        }
        return xySeries;
    }
